...
public class CustomerDAO {
    ...
    public String deleteCustomer(String deleteCustomer) {
        ...
        try {
            Connection conn = DriverManager.getConnection(url, username, password);
            PreparedStatement statement = conn.prepareStatement("DELETE FROM customer WHERE (cid = ?)");
            statement.setString(1, deleteCustomer);
            statement.executeUpdate();
            conn.close();
        } catch (SQLException e) {
            e.printStackTrace();
